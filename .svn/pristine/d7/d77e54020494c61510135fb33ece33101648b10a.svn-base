...
<template>
  <div class="userClouds">
    ...
    <el-table :data="userClouds" style="width:100%" class='table' border stripe :header-row-style="rowClass" size='mini'@selection-change="selectArr" align='center' :height='height'>
...
export default {
  ...
  created(){
    ...
    this.userUpUrl = bp+"/clouds/user_up?token="+this.token;
    this.height = $(window).height() - 210;
  },
  computed:{
  ...
  methods:{
    ...
    download(){//下载模版。
    //跳转到下载地址：
      window.location.href = bp+'/clouds/user_download?token='+this.token;
    },
    ...
    single2(){
      this.visible2=true;
      this.title2='单个新增用户';
      this.form2={user_gender:'女',user_status:'启用',user_tel:'',};
    },
    batch2(){
      this.visible2s=true;
