...
<template>
    ...
    <el-table :data="grade" class='table' border stripe :header-row-style="rowClass" size='mini' :height='height'>
      <el-table-column type="index" label="序号" v-if='false' align='center'style="width: 5%"></el-table-column>
      <el-table-column prop="grade_order" label="年级序号" align='center' width='80'></el-table-column>
    ...
    <el-dialog :visible.sync="visible3" center>
      ...
      <el-form :model="form3" size="mini" :rules='rules' style="height: 100%" class='demo-ruleForm dialog' ref='form3'>
       ...
       <!--  <el-form-item :label-width="fw"> -->
          <el-text style="" id='waring'>*该年级序号涉及到年级升级，请确保序号的唯一性,连续性以及和年级的对应关系，请谨慎
          </el-text>
       <!--  </el-form-item> -->
...
export default {
  ...
  methods:{
    add3(){
      ...
      this.reqGrade().then(()=>{
      //  this.form3.grade_order=this.reqGreade;
        let n = this.reqGreade;
        this.form3 = {grade_status:'启用',grade_graduation:'否',grade_order:n};
      }).catch((error)=>{
          this.$notify({title:'失败', type: 'error', message: '操作失败'});
      });
      // this.form3.grade_order=this.reqGreade[0];
    },
    edit3(i,row){
      this.visible3=true;
      this.title3='年级信息修改';
    ...
    ...mapActions(['findAllGrade','saveGrade','deleteByIdGrade','findGrade','reqGrade']),
  },
}
</script>
