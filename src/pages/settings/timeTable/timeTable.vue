        ...
        <el-col :span='8'>
          <el-row type='flex' justify='end'>
            <el-form-item label='' class='but'>
              <el-button type='info' size='small' @click='save2' class="save" v-if="flag==1">保存</el-button>
  ...
  methods:{
    ...
    submit2(){
      ...
      this.saveTimeTable(obj).then((result)=>{
        ...
        this.findTimeTable({grade_id:grade});
      }).catch((error)=>{
        console.log('课表管理修改操作失败，后台报错！-->'+error);
      });
      this.visible=false;
      this.bool = false;
      this.findTimeTable({grade_id:this.form.grade_id});//继续查询默认年级下的班级。
    },
    rowClass(row, index) {
