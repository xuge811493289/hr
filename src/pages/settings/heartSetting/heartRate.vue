  ...
  <div class="heartRate">
    ...
    <el-table :data="param2" width="100%"  class="table" size='mini' border stripe :header-row-style="rowClass" align='center'>
      ...
      <el-table-column prop="heartrate_value" label="参数" align='center'>
        <template slot-scope='scope'>
          <el-input v-model='scope.row.heartrate_value' type="number"  @blur="heartRateCheck(scope.$index,scope.row.heartrate_value)" size='mini' style='width:80%;' :disabled='bool'></el-input>
        </template>
      </el-table-column>
    ...
    <el-table :data="param3" style="width:100%" size='mini' border stripe :header-row-style="rowClass" align='center' :height='height'>
      <el-table-column prop="heartrate_id" label="heartrate_id" v-if='false' align='center'></el-table-column>
      <el-table-column prop="heartrate_name" label="参数名称" align='center'>
        <template slot-scope='scope'>
          <el-input v-model='scope.row.heartrate_name' size='mini' class='input' :disabled='bool' style='width:80%;'></el-input>
        </template>
      ...
      <el-table-column prop="heartrate_value" label="参数" align='center'>
        <template slot-scope='scope'>
          <el-input v-model='scope.row.heartrate_value' size='mini' style='width:80%;' :disabled='bool' @blur='heartRateCheck2(scope.$index,scope.row.heartrate_value)'></el-input>
        </template>
      </el-table-column>
...
<!-- 
    <el-dialog :title="title2" :visible.sync="visible2">
     ...
     -->
    <el-dialog :visible.sync="visible3" class='dialog3' center>
...
export default {
  ...
  created(){
    this.findAllRate();
    this.height = ($(window).height()-256)/2+40;//应对于不同的分别率。
  },
  computed:{
  ...
  /*  delete2(i,row){
    ...
    heartRateCheck(i,value){
      if(i == 0){//当用户操作最低心率
        if(this.param2[2].heartrate_value){//非空。
          if(+value > +this.param2[2].heartrate_value){
            this.$notify({type:'info',message:'最低心率应小于平均心率，请您重新设置！'}); 
            this.param2[0].heartrate_value = '';
          }
        }
        if(this.param2[1].heartrate_value){
          if(+value > +this.param2[1].heartrate_value){
            this.$notify({type:'info',message:'最低心率应小于最高心率，请您重新设置！'}); 
            this.param2[0].heartrate_value = '';
          }
        }  
      }else if(i == 1){//当用户操作最高心率
        if(this.param2[2].heartrate_value){
          if(+value < +this.param2[2].heartrate_value){
            this.$notify({type:'info',message:'最高心率应大于平均心率，请您重新设置！'}); 
            this.param2[1].heartrate_value = '';
          }
        }
        if(this.param2[0].heartrate_value){
          if(+value < +this.param2[0].heartrate_value){
            this.$notify({type:'info',message:'最高心率应大于最低心率，请您重新设置！'}); 
            this.param2[1].heartrate_value = '';
          }
        }
      }else if(i == 2){//当用户操作平均心率
        if(this.param2[1].heartrate_value){
          if(+value > +this.param2[1].heartrate_value){
            this.$notify({type:'info',message:'平均心率应小于最高心率，请您重新设置！'}); 
            this.param2[2].heartrate_value = '';
          }
        }
        if(this.param2[0].heartrate_value){
          if(+value < +this.param2[0].heartrate_value){
    ...
    //阶段心率值的验证：
    heartRateCheck2(i,value){
      var len = this.param3.length - 1;
      console.log(len)
      if(i == 0){//当用户操作阶段一心率
        if(+value > +this.param3[1].heartrate_value){
          this.$notify({type:'info',message:'阶段1心率应小于阶段2心率，请您重新设置！'}); 
          this.param3[0].heartrate_value = '';
        }
      }else if(i == len){//当用户操作最后一个阶段心率
        if(+value < +this.param3[i-1].heartrate_value){
          this.$notify({type:'info',message:'阶段'+(i+1)+'心率应大于阶段'+i+'心率，请您重新设置！'}); 
          this.param3[i].heartrate_value = '';
        }
      }else{//当用户操作第一个阶段到最后一个阶段的心率
        if(+value > +this.param3[i+1].heartrate_value){
          this.$notify({type:'info',message:'阶段'+(i+1)+'心率应小于阶段'+(i+2)+'心率，请您重新设置！'}); 
          this.param3[i].heartrate_value = '';
        }else if(+value < +this.param3[i-1].heartrate_value){
          this.$notify({type:'info',message:'阶段'+(i+1)+'心率应大于阶段'+i+'心率，请您重新设置！'}); 
          this.param3[i].heartrate_value = '';
        }
      }
