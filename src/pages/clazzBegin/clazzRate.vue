      ...
      <el-col :span='19'>
        <el-row>
          <el-col :span='10' style='height:30px;'>
            <el-row>
              <el-col :span='8' class='font'>{{rateBegin2[2]}}</el-col>
              <el-col :span='8' class='font'>{{rateBegin2[0]}}</el-col>
              <el-col :span='8' class='font'>{{rateBegin2[1]}}</el-col>
            </el-row>
          </el-col>
          <el-col :span='10' class='font' style='height:30px;' v-if='!flagBegin'>
            <el-row>
              <el-col :span='8' v-for='item in rateBegin3' :key='item.heartrate_name'>{{item.name}}：{{item.value}}</el-col>
            </el-row>
          </el-col>
          <el-col :span='10' class='font' style='height:30px;' v-if='flagBegin'>
            <marquee direction='left' behavior='alternate' scrolldelay='100' onmouseover='this.stop()' onmouseout='this.start()'>
              <el-col :span='24' class='font'>
                <span v-for='item in rateBegin3' :key='item.heartrate_name'>
          ...
          <!-- 班级列表 -->
          	<el-row id="rmove">
                ...
                <el-tooltip class="item" effect="dark" placement="bottom-end">
                    ...
                    <div class='maximum'>最高 {{item.heartrate[1][2]}}</div>
                    <div class='minimum'>最低 {{item.heartrate[1][0]}}</div>
                    <div class='star' style='color:#AA0000;'>
                      <i class='el-icon-star-off' v-for='j in item.stars' :key='j.k'></i>
                    </div>
  	              </div>
                  <div slot='content'>平均心率：{{item.heartrate[1][1]}}</div>
                  <div slot='content' v-for='j,k in item.heartrate[2]'>第{{k+1}}阶段心率：{{item.heartrate[2][k]}}</div>
                </el-tooltip>
  	          </el-col>
  			    </el-row>
          </div>
...
//警报音乐：
  //  this.src = 'http://ydown.smzy.com/yinpin/2015-8/smzy_2015080601.mp3';//119
    this.src = 'http://ydown.smzy.com/yinpin/2014-10/smzy_2014102004.mp3';
  //  this.src = 'http://ydown.smzy.com/yinpin/2014-8/smzy_2014082602.mp3';//龙卷风
  //  this.src = 'http://ydown.smzy.com/yinpin/2014-5/smzy_2014051605.mp3';
  //  this.src = 'http://ydown.smzy.com/yinpin/2016-6/smzy_2016062205.mp3';
  },
  computed:{
  ...
  methods:{
    ...
    //  this.form.class_id = '';  //这么写不行，会导致以后再怎么选也选不上。
      this.reqClazzBegin({grade_id:gid}).then(()=>{
    //    this.form.class_id = this.clazzBegin[0].class_id;//默认给每个年级加第一个班级。
      }).catch((error)=>{
        this.$notify({title:'失败',type:'error',message:'操作失败'});
      });
      let vm = this; 
    ...
    //开始上课：
  	startSession(){
      if(this.form.grade_id && this.form.class_id && this.form.tools_id && this.form.course_type_id && this.bind.length>0){//手环绑定成功执行，有数据。
